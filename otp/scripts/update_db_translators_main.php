...
while ($page <= $last_page){
	...
	if (($curl_error == 0) && (intval($response_info['http_code']) == 200)) {
		...
		if ($curl_error > 0) {
			otp_error("Amara webpage connection error CURL $curl_error page: hu translator team");
		}
...
if (count($translator_data) > 0) {
	...
	if($db->connect_errno > 0){
		...
		$sql = "SELECT * FROM otp_languages WHERE language_name IN (" .implode(',', $sql_list) .")";
		if ($result = $db->query($sql)) {
