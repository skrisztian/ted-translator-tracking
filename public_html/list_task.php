...
echo '<h1>What is going on? The long version</h1>';
...
$header = 'Finished translation, waiting for review';
$sql = "SELECT t.*, tr.full_name, s.title_orig, c.name FROM otp_tasks t 
		LEFT OUTER JOIN otp_translators tr ON t.assignee=tr.amara_id 
		LEFT OUTER JOIN otp_subtitles s ON t.video_id=s.video_id 
		LEFT OUTER JOIN otp_task_codes c ON t.approved=c.id 
...
$header = 'Waiting for translation';
$sql = "SELECT t.*, tr.full_name, s.title_orig, c.name FROM otp_tasks t 
		LEFT OUTER JOIN otp_translators tr ON t.assignee=tr.amara_id 
		LEFT OUTER JOIN otp_subtitles s ON t.video_id=s.video_id 
		LEFT OUTER JOIN otp_task_codes c ON t.approved=c.id 
		WHERE t.type='Translate' AND t.completed IS NULL AND t.assignee IS NULL 
		ORDER BY t.id DESC";
print_table($db, $sql, $header);

$header = 'Waiting for subtitles';
$sql = "SELECT t.*, tr.full_name, s.title_orig, c.name FROM otp_tasks t 
		LEFT OUTER JOIN otp_translators tr ON t.assignee=tr.amara_id 
		LEFT OUTER JOIN otp_subtitles s ON t.video_id=s.video_id 
		LEFT OUTER JOIN otp_task_codes c ON t.approved=c.id 
		WHERE t.type='Subtitle' AND t.completed IS NULL AND t.assignee IS NULL 
		ORDER BY t.id DESC";
print_table($db, $sql, $header);

$header = 'Already published';
$sql = "SELECT t.*, tr.full_name, s.title_orig, c.name FROM otp_tasks t 
		LEFT OUTER JOIN otp_translators tr ON t.assignee=tr.amara_id 
		LEFT OUTER JOIN otp_subtitles s ON t.video_id=s.video_id 
		LEFT OUTER JOIN otp_task_codes c ON t.approved=c.id 
		WHERE t.type='Approve' AND t.completed IS NOT NULL AND t.assignee IS NOT NULL 
